...
BOOKMARK_PATH = 'Library/Application Support/Sidekick/Default/Bookmarks'
BOOKMARKS_MAP = {
...
def get_all_urls(url_data: dict(str, str)) -> list(tuple):
    """
    Extract all URLs and title from Bookmark files

    Args:
        url_data (str): All Bookmarks read from file

    Returns:
        list(tuple): List of tuple with Bookmarks url and title
    """
    ...
    for data in url_data.values():
        data.get('type')

    get_container(url_data)
...
def get_url_data_from_file(file: str) -> dict:
    """
    Get Bookmark JSON
...
def main():
    ...
    path_to_bm = os.path.join(os.path.expanduser('~'), BOOKMARK_PATH)
    bm_data = get_url_data_from_file(path_to_bm)
    bookmarks = get_all_urls(bm_data)
    Tools.log("############")
    pprint(bm_data)
    # Tools.log(bm_data)
    ...
    if len(bms) > 0:
        ...
        for bookmarks_file in bms:
            bm_data = get_url_data_from_file(bookmarks_file)
            bookmarks = get_all_urls(bm_data)
